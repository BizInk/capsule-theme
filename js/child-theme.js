	...
	jQuery(document).ready(function ($) {
	  ...
	  if (jQuery(window).width() > 767) {
	    jQuery(window).on('load resize', function () {
	      ...
	      jQuery('.services-infobox .info-description').each(function () {
	        if (infoboxDescription < jQuery(this).outerHeight()) {
	          infoboxDescription = jQuery(this).outerHeight();
	        }
	      });
	      jQuery('.checklist-infobox .info-description').each(function () {
	        if (checklistDescription < jQuery(this).outerHeight()) {
	          checklistDescription = jQuery(this).outerHeight();
	        }
	      }); //Calculating the description new height

	      jQuery('.services-infobox .info-description').css({
	        'min-height': infoboxDescription
	      });
	      jQuery('.checklist-infobox .info-description').css({
	        'min-height': checklistDescription
	      });
	    });
	  } // Video Popup Close and Play
